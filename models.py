...
class Book(db.Model):
    ...
    type_of_cover = db.Column(db.String(200))
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self, book_title, author_name, book_length, type_of_cover, user_token, id=''):
        self.id = self.set_id()
    ...
    def __repr__(self):
        return f'The following book has been added to the checkout: {self.book_title}'
...
class BookSchema(ma.Schema) :
    class Meta:
        fields = ['id', 'book_title', 'author_name', 'book_length', 'type_of_cover'] 
